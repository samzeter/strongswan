...
{
	transform_type_t t = DIFFIE_HELLMAN_GROUP;
	uint16_t alg;
	int i = 1;
...
static void process_ke_payload(private_ike_init_t *this, ke_payload_t *ke)
{
	...
	if (method != this->ke_method)
	{
		DBG1(DBG_IKE, "key exchange method in received payload %N doesn't "
			 "match negotiated %N", key_exchange_method_names, this->ke_method,
	...
	{
		this->ke_failed = this->ke->get_method(this->ke) != this->ke_method;
	}
...
/**
 * Build payloads during IKE_INTERMEDIATE exchanges
 */
static bool build_payloads_intermediate(private_ike_init_t *this,
										message_t *message)
{
	ke_payload_t *ke;
	...
	nonce->set_nonce(nonce, this->my_nonce);
	...
	private_ike_init_t *this, message_t *message)
{
	...
	if (!build_payloads_intermediate(this, message))
	{
		return FAILED;
	}
	return NEED_MORE;
...
/**
 * Process payloads in IKE_INTERMEDIATE exchanges
 */
static void process_payloads_intermediate(private_ike_init_t *this,
										  message_t *message)
{
	ke_payload_t *ke;
	nonce_payload_t *nonce;

	chunk_clear(&this->other_nonce);
	...
	private_ike_init_t *this, message_t *message)
{
	...
	{
		DESTROY_IF(this->ke);
		chunk_clear(&this->my_nonce);
		if (!generate_nonce(this))
		{
			return FAILED;
		}
		process_payloads_intermediate(this, message);
	}
	return NEED_MORE;
}
						...
						chunk_t nonce_i, chunk_t nonce_r)
{
	...
	if (!this->keymat->derive_ike_keys(this->keymat, this->proposal, this->ke,
	...
	charon->bus->ike_keys(charon->bus, this->ike_sa, this->ke, chunk_empty,
...
METHOD(task_t, post_build_r, status_t,
	private_ike_init_t *this, message_t *message)
{
	/* key derivation for additional key exchanges is like rekeying, so set our
	 * own SA as old SA to get SK_d */
	this->old_sa = this->ike_sa;
	if (!derive_keys(this, this->other_nonce, this->my_nonce))
	{
		DBG1(DBG_IKE, "%N key derivation failed", transform_type_names,
			 this->key_exchanges[this->ke_index].type);
		return FAILED;
	}
...
METHOD(task_t, build_r_intermediate, status_t,
	private_ike_init_t *this, message_t *message)
{
	if (!this->ke || !this->other_nonce.ptr)
	{
	...
	private_ike_init_t *this, message_t *message)
{
	...
	{	/* use IKE_INTERMEDIATE for additional key exchanges */
	...
	private_ike_init_t *this, message_t *message)
{
	...
	if (this->ke_failed || !this->other_nonce.ptr)
	{
		return FAILED;
	}
